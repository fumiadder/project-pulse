...
export function WeeklyHeatmap({ todos, weeks = 8 }: WeeklyHeatmapProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-1.5">
        ...
        <div className="flex flex-col gap-1 pt-4">
          {weekdayLabels.map((d, i) => (
            <div
              key={i}
              className="flex h-3.5 w-3 items-center justify-center text-[8px] text-text-muted"
            >
              {i % 2 === 0 ? d : ''}
            </div>
          ))}
          ...
          <div className="flex gap-1 mb-0.5">
            {grid.map((_, wi) => {
              ...
              return (
                <div
                  key={wi}
                  className="w-3.5 text-[8px] text-text-muted"
                  style={{ minWidth: '14px' }}
                >
                  {label?.label ?? ''}
                </div>
            ...
            {grid.map((week, wi) => (
              <div key={wi} className="flex flex-col gap-1">
                {week.map((cell) => (
                  <div
                    key={cell.date}
                    className={`h-3.5 w-3.5 rounded-sm transition-all hover:ring-1 hover:ring-accent-cyan/50 ${getCellColor(cell)} ${
      ...
      {/* 图例 */}
      <div className="flex items-center justify-end gap-1.5 text-[8px] text-text-muted">
        <span>少</span>
        <div className="h-2.5 w-2.5 rounded-sm bg-bg-tertiary/50" />
        <div className="h-2.5 w-2.5 rounded-sm bg-accent-green/30" />
        <div className="h-2.5 w-2.5 rounded-sm bg-accent-green/50" />
        <div className="h-2.5 w-2.5 rounded-sm bg-accent-green/70" />
